    ...
    <>
      ...
      <div class={styles().hotkeyList}>
        <For each={sequenceRegistrations()}>
          {(reg) => {
            const targetConflicts = () =>
              findSequenceTargetConflicts(reg, sequenceRegistrations())
            const scopeConflicts = () =>
              findSequenceScopeConflicts(reg, sequenceRegistrations())
            ...
            const triggerCount = () =>
              sequenceRegistrations().find((r) => r.id === reg.id)
            ...
            return (
              ...
              >
                <span class={styles().hotkeyLabel}>
                  {formatHotkeySequence(reg.sequence)}
                </span>
                <Show when={triggerCount() > 0}>
                  <span class={styles().triggerCount}>x{triggerCount()}</span>
                </Show>
